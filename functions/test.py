...
def run_test(event, vk, ids_data):
    message_text = event.obj.message['text'].lower()
    if ids_data[event.obj.message['from_id']]['test_question'] == 0:
        ...
        ids_data[event.obj.message['from_id']]['test_question'] += 1
    elif ids_data[event.obj.message['from_id']]['test_question'] == 1:
        time.sleep(1)
        if "4" in message_text or 'четыре' in message_text:
            vk.messages.send(user_id=event.obj.message['from_id'],
                             message="Верно!",
                             random_id=random.randint(0, 2 ** 64))
            ids_data[event.obj.message['from_id']]['score'] += 1
        else:
            vk.messages.send(user_id=event.obj.message['from_id'],
        ...
        vk.messages.send(user_id=event.obj.message['from_id'],
                         ...
                         keyboard=keyboards.new_keyboard(event, ids_data).get_keyboard())
        ids_data[event.obj.message['from_id']]['test_question'] += 1
    elif ids_data[event.obj.message['from_id']]['test_question'] == 2:
        time.sleep(1)
    ...
    elif ids_data[event.obj.message['from_id']]['test_question'] == 3:
        time.sleep(1)
        if "300" in message_text or 'триста' in message_text:
            ...
            vk.messages.send(user_id=event.obj.message['from_id'],
                             ...
                             random_id=random.randint(0, 2 ** 64))
        time.sleep(1)
        ids_data[event.obj.message['from_id']]['test_active'] = False
        ids_data[event.obj.message['from_id']]['test_question'] = 0
        vk.messages.send(user_id=event.obj.message['from_id'],
                         message=f"Тест закончен!\n"
                                 f"Вы набрали {ids_data[event.obj.message['from_id']]['score']} баллов из 3!",
                         random_id=random.randint(0, 2 ** 64),
                         keyboard=keyboards.new_keyboard(event, ids_data).get_keyboard())
        if ids_data[event.obj.message['from_id']]['score'] == 3:
            time.sleep(1)
            ...
            if 'sex' in user:
                ...
                vk.messages.send(user_id=event.obj.message['from_id'],
                                 message=f"Вы молодец!",
                                 random_id=random.randint(0, 2 ** 64))
        ids_data[event.obj.message['from_id']]['score'] = 0
